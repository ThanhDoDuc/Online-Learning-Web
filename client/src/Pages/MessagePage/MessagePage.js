...
const MessagePage = () => {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      const res = await MessageServices.getAllUserInConversation(user.id);
  ...
  const fetchMessages = async (conversationId, receiver) => {
    const res = await MessageServices.getMessage(
      conversationId,
      user.userId,
      receiver?.receiverId
    );
          ...
          {messages?.receiver?.name && (
            <div className="p-14 w-full flex items-center">
              <input
                ...
                className="w-[75%] p-4 border-0 shadow-md rounded-full bg-light focus:ring-0 focus:border-0 outline-none"
              />
              <div
